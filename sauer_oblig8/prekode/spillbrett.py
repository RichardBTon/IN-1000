...
class Spillbrett:
    ...
    def legg_til_objekter_fra_fil(self, filnavn):
        ...
        # Leser filen, og lager en liste med objekter som skal vises
        with open(filnavn) as f:
            ...
            for line in f:
                ...
                game_object = {
                    "type": object[0],
                    "koordinater": (int(object[1]), int(object[2]))
                }
                game_objects.append(game_object)

        # Legger til riktige objekter i spillbrettet
        for object in game_objects:
            print(object["type"], object["koordinater"])
            if object["type"] == "sau":
                self.opprett_sau(
                    "sau", object["koordinater"][0], object["koordinater"][1])
            elif object["type"] == "ulv":
                self.opprett_ulv(
                    "ulv", object["koordinater"][0], object["koordinater"][1])
            elif object["type"] == "stein":
                self.opprett_stein(
                    "stein", object["koordinater"][0], object["koordinater"][1])
            elif object["type"] == "gress":
                self.opprett_gress(
                    "gress", object["koordinater"][0], object["koordinater"][1])
            else:
                raise TypeError("Ugyldig objekt i banen.")
